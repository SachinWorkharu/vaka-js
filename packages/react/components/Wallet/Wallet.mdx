...
These components only work when used with the following context/providers in `@vaka-consulting/react` package:
...
```
import { WalletProvider, WalletExtendedProvider } from '@vaka-consulting/react'
...
```
import { useWallet, useExtendedWallet } from '@vaka-consulting/react'
